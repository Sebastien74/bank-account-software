...
export function scrollEvent(nav) {
    ...
    const scrollLimit = nav.clientHeight + 45;
    ...
    window.addEventListener('scroll', function () {
        ...
        if (nav && scrollDirection === 'down' && Math.abs(scrollPosition) >= scrollLimit && !nav.classList.contains('bottom-shadow')) {
            nav.classList.add('bottom-shadow');
            nav.classList.add('as-scroll');
            body.classList.add('menu-as-scroll');
        } else if (nav && scrollDirection === 'up' && Math.abs(scrollPosition) < (scrollLimit - 100) && nav.classList.contains('bottom-shadow')) {
