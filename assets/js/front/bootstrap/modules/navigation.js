...
export default function () {
    ...
    /** Collapse events */
    import(/* webpackPreload: true */ './navigation-functions').then(({collapseEvent: CollapseEvent}) => {
    ...
    if (dropdownHover || dropdownClick) {
        import(/* webpackPreload: true */ './navigation-functions').then(({dropdownHoverAndClick: DropdownHoverAndClick}) => {
    ...
    if (lateralMenu) {
        import(/* webpackPreload: true */ './navigation-functions').then(({lateralMenu: LateralMenu}) => {
            new LateralMenu(body, lateralMenu, screenWidth);
        }).catch(error => console.error(error.message));
    } else {
        import(/* webpackPreload: true */ './navigation-functions').then(({dropdownManagement: DropdownManagement}) => {
    ...
    if (haveAnchors) {
        import(/* webpackPreload: true */ './navigation-functions').then(({anchorsEvents: AnchorsEvents}) => {
            new AnchorsEvents();
        }).catch(error => console.error(error.message));
    }
