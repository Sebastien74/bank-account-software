...
export default function () {
    ...
    function sendRequest(event, form) {
        ...
        let beforeSend = function () {
            /** Remove errors */
            import(/* webpackPreload: true */ '../../../../vendor/components/remove-errors').then(({default: removeErrors}) => {
                new removeErrors();
            }).catch(error => console.error(error.message));
        ...
        xHttp.onload = function () {
            if (this.readyState === 4 && this.status === 200) {
                ...
                formsEvents();
                import(/* webpackPreload: true */ '../../../../vendor/components/keyup-fields').then(({default: keyupFields}) => {
                    new keyupFields();
                }).catch(error => console.error(error.message));
                if (response.success) {
                    resetInputs();
