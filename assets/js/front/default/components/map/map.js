...
export default function (maps, autoCenterMap = null) {
    ...
    let iniMap = function (mapBox) {
        ...
        if (autoCenter && allMarkers.length > 0) {
            ...
            map.flyToBounds(markers.getBounds(), {duration: 1, padding: [50, 50], maxZoom: parseInt(data.dataset.zoom)})
        }
        ...
        function html(point, data, marker) {
            ...
            let html = '';
            let display = false;
            let htmlInViewEl = point.getElementsByClassName('point-popup-html');

            if (htmlInViewEl.length > 0) {
                let elHTML = htmlInViewEl[0];
