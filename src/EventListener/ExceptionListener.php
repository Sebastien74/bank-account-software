...
class ExceptionListener
{
    ...
    public function onKernelException(ExceptionEvent $event): void
    {
        ...
        if ($asUnknownError && $request->getSession()->get('mainExceptionMessage') && !$this->isDoctrineError($exception)) {
        ...
        if ($exception instanceof AccessDeniedHttpException) {
        ...
        } elseif ($allowedIP && $this->isDoctrineError($exception)) {
            ...
            if ($havePrevious && method_exists($exception->getPrevious(), 'getMessage')) {
                ...
                if ($allowedIP && $this->isDoctrineError($exception->getPrevious())) {
                    $logger = new Logger('doctrine-update-errors');
                    $logger->pushHandler(new RotatingFileHandler($this->coreLocator->logDir().'/doctrine-update-errors.log', 10, Level::Critical));
                    $logger->critical($exception->getMessage());
     ...
     */
    private function isDoctrineError(mixed $exception): bool
    {
