...
class FormManager
{
    ...
    public function getContact(): mixed
    {
        ...
        if ($token) {
            $contact = $this->coreLocator->em()->getRepository(ContactForm::class)->findOneBy(['token' => $token, 'tokenExpired' => false]);
            if (!$contact) {
                $contact = $this->coreLocator->em()->getRepository(ContactStepForm::class)->findOneBy(['token' => $token, 'tokenExpired' => false]);
            }
            if (!$contact) {
     ...
     */
    public function success(StepForm|Form $form, FormInterface $formPost): bool|ContactForm|ContactStepForm
    {
     ...
     */
    private function checkContact(StepForm|Form $form): bool
    {
        if ($form->getConfiguration()->isUniqueContact() && ($this->senderInForm || $this->phone)) {
            if ($this->senderInForm) {
                $existing = $this->coreLocator->em()->getRepository(ContactForm::class)->findOneBy([
                    'form' => $form,
                    'email' => $this->sender,
                ]);
            } else {
     ...
     * @throws Exception
     */
    private function addContact(StepForm|Form $form): ContactForm|ContactStepForm|null
    {
        $registrationValid = $this->checkRegistration($form);

        if ($registrationValid) {
            $contact = $form instanceof Form ? new ContactForm() : new ContactStepForm();

            $requestCalendar = !empty($_GET['calendar']) ? $_GET['calendar'] : null;
            if ($requestCalendar) {
                $contact->setCalendar($this->coreLocator->em()->getRepository(Calendar::class)->find($requestCalendar));
            }
            ...
            foreach ($this->fields as $keyName => $field) {
                ...
                if ($fieldValue instanceof \DateTime) {
                    ...
                    $setField = true;
                    if ($fieldValue && Calendar::class === $fieldConfiguration->getClassName() && $fieldValue->getId()) {
                        $contact->setCalendar($this->coreLocator->em()->getRepository(Calendar::class)->find($fieldValue->getId()));
                    }
     ...
     * @throws Exception
     */
    private function checkRegistration(StepForm|Form $form): bool
    {
        $configuration = $form->getConfiguration();
     ...
     * Set attachments.
     */
    private function setAttachments(StepForm|Form $form, ContactForm|ContactStepForm|null $contact = null): void
    {
        $flushContact = false;
     ...
     * To upload File.
     */
    private function uploadedFile(array $field, string $keyName, StepForm|Form $form, UploadedFile $fieldValue, ContactForm|ContactStepForm|null $contact = null): bool
    {
        $flushContact = false;
     ...
     */
    private function sendEmail(WebsiteModel $website, StepForm|Form $form, mixed $intl): void
    {
     ...
     * @throws ExceptionInterface|Exception
     */
    private function sendConfirm(WebsiteModel $website, StepForm|Form $form, mixed $intl): void
    {
