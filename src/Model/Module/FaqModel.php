...
final class FaqModel extends BaseModel
{
     ...
     */
    public static function fromEntity(CoreLocatorInterface $coreLocator, ?Block $block = null, mixed $filter = null): object
    {
        $website = self::$coreLocator->website();
        $faq = self::$coreLocator->em()->getRepository(Faq::class)->findOneByFilter($website->entity, self::$coreLocator->locale(), $filter);
        ...
        return (object) array_merge($model, [
            'entity' => $faq,
            'questions' => $questions,
            'microdata' => $faq ? self::microdata($website, $faq, $questions, $block) : [],
        ]);
    }
     ...
     * @throws MappingException|NonUniqueResultException
     */
    private static function microdata(WebsiteModel $website, Faq $faq, array $questions, ?Block $block = null): array
    {
        $microdata = [];
                    ...
                    foreach ($col->getBlocks() as $block) {
                        foreach ($block->getActionIntls() as $actionIntl) {
                            if ($block->getAction() && FaqController::class === $block->getAction()->getController()
                                ...
                                self::$microdata = true;
                                $faqAction = self::$coreLocator->em()->getRepository(Faq::class)->findOneByFilter($website->entity, self::$coreLocator->locale(), $actionIntl->getActionFilter());
