...
final class MediaModel extends BaseModel
{
    ...
    public static function fromEntity(mixed $entity, CoreLocatorInterface $coreLocator, bool $query = true): self
    {
        ...
        if ($entity && isset($cache['response'][get_class($entity)][$entity->getId()][self::$coreLocator->locale()])) {
            return $cache['response'][get_class($entity)][$entity->getId()][self::$coreLocator->locale()];
        }

        $metadata = $entity ? self::$coreLocator->em()->getClassMetadata(get_class($entity)) : null;
        ...
        $website = !self::$website && $media ? $media->getWebsite() : self::$website;
        $websiteDirname = $website ? $website->getUploadDirname() : null;
        $intl = $mediaRelation ? IntlModel::fromEntity($mediaRelation, $coreLocator) : null;
