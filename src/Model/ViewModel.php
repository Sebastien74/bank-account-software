...
final class ViewModel extends BaseModel
{
    ...
    public function __construct(
        ...
        public readonly ?Url $urlEntity = null,
        ...
        public readonly ?Layout\Layout $layout = null,
    ...
    public static function fromEntity(mixed $entity, CoreLocatorInterface $coreLocator, array $options = []): self
    {
        ...
        $url = !$disabledUrl ? self::url($entity, $interface, $locale, $urlsIndex) : (object) [];
        ...
        return new self(
            ...
            urlIndex: $entity && !empty($urlsIndex[$entity->getId()]) ? $urlsIndex[$entity->getId()] : self::$coreLocator->request()->get('pageUrl'),
            interface: $interface,
     ...
     * Get url.
     */
    public static function url(mixed $entity, array $interface = [], ?string $locale = null, array $urlsIndex = []): bool|object
    {
        if (!$entity) {
            return false;
        }
        ...
        if (!empty(self::$cache[get_class($entity)]['url'][$entity->getId()])) {
    ...
    private static function preload(mixed $entity, mixed $layout): array
    {
        ...
        $files = $preloadFiles = $videosFiles = [];
        $route = self::$coreLocator->request()->get('_route');
        ...
        if ($preload && is_object($layout) && property_exists($layout, 'entity')) {
            ...
            // To preload assets
            if (empty(self::$cache['preloads'])) {
                $preloadFiles = self::$coreLocator->preloadFiles();
                foreach ($preloadFiles['js'] as $file) {
                    $linkProvider = self::$coreLocator->request()->attributes->get('_links', new GenericLinkProvider());
                    self::$coreLocator->request()->attributes->set('_links', $linkProvider->withLink(
                        (new Link('preload', $file))->withAttribute('as', 'javascript')
                    ));
                }
            }
        }

        self::$cache['preloads'] = array_merge(['images' => $files], ['videos' => $videosFiles], $preloadFiles);
