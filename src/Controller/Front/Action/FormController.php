...
class FormController extends FrontController
{
     ...
     * Get redirection.
     */
    private function getRedirection(Request $request, Url $url, Configuration $configuration, $contact = null): RedirectResponse|bool|string|null
    {
        if (!$contact) {
            return false;
        }
        ...
        if ($configuration->isThanksPage()) {
            ...
            return $this->generateUrl('front_form_thanks', ['code' => $configuration->getForm()->getSlug(), 'token' => $contact->getToken()]);
        }
