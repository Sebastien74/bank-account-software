...
//        $this->products = array_column($products, 'id');
    ...
    private function getByCategories(mixed $catalogs): array|Collection
    {
        ...
        // to find all categories if need
            ...
            if (is_iterable($fields)) {
                foreach ($fields as $key => $value) {
                    foreach ($this->products as $product) {
                        ...
                        } elseif ('subcategories' === $key || 'filters' === $key && is_array($value) && !empty($value['subcategories'])) {
    ...
    private function result(array $productIds, array $searchResult, array $data): array
    {
        ...
        $subCategories = !empty($data['subcategories']) ? $data['subcategories'] : (!empty($data['filters']['subcategories']) ? $data['filters']['subcategories'] : null);
        if ($subCategories) {
            foreach ($this->products as $key => $product) {
                if (empty($productIds['subcategories'][$subCategories][$product->getId()])) {
                    unset($searchResult[$key]);
                }
            }
        }
