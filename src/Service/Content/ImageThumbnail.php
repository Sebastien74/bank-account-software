...
class ImageThumbnail implements ImageThumbnailInterface
{
    ...
    public function execute(?MediaModel $mediaModel = null, array $thumbs = [], array $options = [], bool $generator = false): mixed
    {
        ...
        if ($media instanceof Media\Media && $media->getFilename()) {
            ...
            if ($extension && ('desktop' === $media->getScreen() || 'poster' === $media->getScreen())) {
                foreach ($this->screensSizes as $size) {
                    ...
                    $mediaRelation = $options['mediaRelation'] = !empty($options['mediaRelation']) ? $options['mediaRelation'] : ($asMediaModel ? $mediaModel->mediaRelation : null);
                    $dirname = $fileDirname ?: (str_contains($media->getFilename(), '/build/')
                    ...
                    $dirname = $this->filesystem->exists($dirname) ? $dirname : $this->dirname($this->projectDirname.'/public/medias/placeholder.jpg');
        ...
        $sizesDisplay = $thumbnails['sizesDisplay'] = self::SCREENS_SIZES[$this->screen];
        ...
        //        if (!empty($thumbnails['files']) && $originalDirname && !$lazyLoad && $mediaModel->extension && !in_array($mediaModel->extension, self::EXCEPTIONS_EXTENSIONS)) {
        //            foreach ($thumbnails['files'] as $screenSize => $file) {
        //                if (!in_array($screenSize, $sizesDisplay) || in_array($screenSize, self::RETINA_SIZES)) {
        //                    unset($thumbnails['files'][$screenSize]);
        //                }
        //            }
        //        }
    ...
    private function setThumbInfos(
        ...
        ?int $height = null,
        array $options = [],
        ?Media\ThumbConfiguration $thumbConfiguration = null): object
    {
    ...
    private function setRatio(object $mediaRelation, object $thumbInfos, int $size, array $options = []): object
    {
        ...
        //        $isRetinaSize = in_array($size, self::RETINA_SIZES);
        //        $info = $options['sizeInfo'];
        //
        //        if ($colSize && $colSize < 12 && in_array($size, self::SCREENS_SIZES['desktop'])) {
        //            $colRatio = 12 / $colSize;
        ...
        //            $this->cache['ratioMaxWidth'][$size] = $ratioMaxWidth < $initCropWidth ? $ratioMaxWidth : null;
        //            if (!$asCrop && $ratioMaxWidth < $initCropWidth) {
        //                //            if ($ratioMaxWidth < $initCropWidth) {
        //                $width = $thumbConfiguration->getWidth() >= $ratioMaxWidth ? $ratioMaxWidth : (int) ceil(($thumbConfiguration->getWidth() * $colSize) / 12);
        //                $height = $width > 0 ? (int) ceil(($thumbConfiguration->getHeight() * $width) / $initCropWidth) : null;
        //                $imgMinSize = $size / $colRatio;
        //                if ($width > 0 && $width < $imgMinSize) {
        //                    $ratio = $imgMinSize / $width;
        //                    $ratio = $isRetinaSize ? $ratio / 2 : $ratio;
        //                    $width = (int) ceil($width * $ratio);
        //                    $height = (int) ceil($height * $ratio);
        //                    //                    $thumb->setScale($ratio);
        //                }
        //                if (is_numeric($info->getWidth()) && $info->getWidth() > 0 && $width > $info->getWidth()) {
        //                    $width = $info->getWidth();
        //                    $height = $info->getHeight();
        //                }
        //                $zoneFullSize = $options['zoneFullSize'] ?? false;
        //                if (!$zoneFullSize && $width && $height) {
        //                    $screenSizes = array_values(self::SCREENS_SIZES[$this->screen]);
        //                    $screensSize = array_shift($screenSizes);
        //                    $width = (int) ceil(($width * $containerSize) / $screensSize);
        //                    $height = (int) ceil(($height * $containerSize) / $screensSize);
        //                    $colWidth = (int) ceil($containerSize / (12 / $colSize));
        //                    if ($width > $colWidth) {
        //                        $height = (int) ceil(($colWidth * $height) / $width);
        //                        $width = $colWidth;
        //                    }
        //                }
        //                //                $thumb->setWidth($width);
