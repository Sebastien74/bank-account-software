...
class SeoService implements SeoInterface
{
    ...
    public function execute(
        ...
        $this->classname = $this->entity ? str_replace('Proxies\__CG__\\', '', get_class($this->entity->entity)) : null;
    ...
    private function getResponse(Seo\Url $url, bool $onlyTitle = false, bool $asIndexMicrodata = false): array
    {
        ...
        return [
            ...
            'createdAt' => method_exists($this->entity->entity, 'getCreatedAt') ? $this->entity->entity->getCreatedAt() : null,
            'updatedAt' => method_exists($this->entity->entity, 'getUpdatedAt') ? $this->entity->entity->getUpdatedAt() : null,
            'publishedDate' => method_exists($this->entity->entity, 'getPublicationDate') && $this->entity->entity->getPublicationDate() ? $this->entity->entity->getPublicationDate()
                : (method_exists($this->entity->entity, 'getPublicationStart') ? $this->entity->entity->getPublicationStart() : null),
    ...
    public function getAsCardUrl(mixed $url, mixed $entity, string $classname, bool $asObject = false, array $interface = [], array $indexPagesCodes = []): object|string|null
    {
        ...
        if (!$domain && $isObjectUrl) {
            $domains = $this->coreLocator->em()->getRepository(Domain::class)->findBy([
                'configuration' => $url->getWebsite()->getConfiguration(),
                'locale' => $locale,
                'asDefault' => true,
            ]);
